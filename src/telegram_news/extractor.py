...
KOREAN_STOCK_KEYWORDS = {
    "반도체": ["반도체", "hbm", "dram", "낸드", "파운드리", "삼성전자", "sk하이닉스"],
...
CRYPTO_KEYWORDS = {
    ...
    "etf": ["etf", "현물 etf", "spot etf", "자금유입", "순유입", "순유출"],
    "defi": ["defi", "디파이", "tvl", "스테이킹"],
    "ai_coin": ["ai", "agent", "에이전트", "인공지능"],
    "rwa": ["rwa", "토큰화", "real world asset"],
}
...
def extract_signals(text: str, repeat_count: int = 1) -> ExtractedSignal:
    ...
    if any(w in lower for w in ["단독", "속보", "수주", "계약", "승인", "상장", "sec", "fed", "fomc"]):
        score += 3
