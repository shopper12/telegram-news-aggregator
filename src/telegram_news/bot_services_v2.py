...
def _code_from_symbol(symbol: str) -> str | None:
    match = re.match(r"^(\d{6})(?:\.(?:KS|KQ))?$", symbol, re.IGNORECASE)
...
def _quote_candidates(query: str) -> list[str]:
    ...
    for code in _kr_code_matches(q):
        out.extend([f"{code}.KS", f"{code}.KQ", code])
    return out
...
def _load_quote_item(symbol: str) -> dict[str, Any] | None:
    item = _yahoo_history(symbol)
    if item:
        return item
    code = _code_from_symbol(symbol)
    return _pykrx_history(code) if code else None
